...
//==== 传入 offset 传回干支, 0=甲子
export function cyclical(num) { return (Gan[num % 10] + Zhi[num % 12]) }
...
export function leapMonth(y) { return (lunarInfo[y - 1900] & 0xf) }
...
export function solarDay1() {
    var sDObj = new Date(SY, SM, SD);
    var lDObj = new Lunar(sDObj);
    var cl = '';
    var tt = '属相：' + Animals[(SY - 4) % 12] + '&nbsp;' + cyclical(lDObj.monCyl) + '月 ' + cyclical(lDObj.dayCyl++) + '日';
    return (cl + tt + '');
}
export function solarDay2() {
    var sDObj = new Date(SY, SM, SD);
    var lDObj = new Lunar(sDObj);
    var cl = '';
    //农历BB'+(cld[d].isLeap?'闰 ':' ')+cld[d].lMonth+' 月 '+cld[d].lDay+' 日
    var tt = '农历' + cyclical(SY - 1900 + 36) + '年 ' + cDay(lDObj.month, lDObj.day);
    return (cl + tt + '');
}
export function solarDay3() {
